...
class Vgg16:
    def __init__(self, vgg16_npy_path=None):
        if vgg16_npy_path is None:
            ...
            print("这是第一路劲",path)
            path = os.path.abspath(os.path.join(path, os.pardir))#返回上一级目录
            print("这是第二路劲",os.pardir)

            path = os.path.join(path, "vgg16.npy")# 把vgg16.npy放到文件目录就行
    ...
    def get_conv_filter(self, name):
        return tf.constant(self.data_dict[name][0], name="filter") #这里从npy文件导入数据
